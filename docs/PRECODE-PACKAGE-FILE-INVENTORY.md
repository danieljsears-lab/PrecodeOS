...
| `scripts/clarity-scenario-check.py` | Runs deterministic beginner-decision, recovery, command-boundary, facade-boundary, and Ralph-boundary scenario fixtures. | In-memory bead scenarios, recovery scenario contracts, Bugfix Spec Lane text-contract checks, stable-fix eligibility scenarios, adaptive-depth scenarios, fallback prompt text-contract checks, command-risk examples, optional CLI delegation checks, and Ralph refusal checks. | Advisory JSON pass/fail result; synthetic recovery fixtures and text-contract checks are regression coverage only, not real recovery evidence, repair approval, rollback approval, setup/update approval, transition approval, command approval, support-bot authority, or external-system permission. |
