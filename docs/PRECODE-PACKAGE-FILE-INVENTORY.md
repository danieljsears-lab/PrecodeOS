...
| Public generated docs site | `docs-html/*.html` | Committed HTML reading surface generated from `docs/*.md` and selected root public docs such as `CANDIDATE-QUEUE.md`; easier to navigate, but not authority. |
...
| Candidate Queue | `CANDIDATE-QUEUE.md` | User-maintained parked intent, candidate evidence, review ordering, status, and promotion targets before PRDs or beads; not task authority. |
...
| Candidate Queue | Parked intent and promotion review | Load when a user wants to park ideas, review candidate evidence, rank candidates for review, or route candidates toward intake, discovery, PRDs, decisions, authority updates, or decomposition. | Preserve intents the project has not lost with enough evidence/status to decide what deserves promotion. | Do not choose the active task, approve PRDs, activate beads, reserve bead IDs, authorize implementation, become active memory, or let generated reports rank/promote candidates. | Promote reviewed conclusions through Local Source Intake, Product Discovery, PRDs, `DECISIONS.md`, owner files, decomposition review, defer, or kill decisions. |
...
| `docs-html/*.html` | generated public docs | Static HTML reading surface generated from `docs/*.md` and selected root public docs such as `CANDIDATE-QUEUE.md`, including a curated compass and per-page navigation. | Commit-eligible package artifact for easier reading; does not replace Markdown authority, approve work, promote Candidate Queue entries, approve PRDs, activate beads, or become active memory. |
...
| `tasks/reference/VERIFICATION-GUARDRAIL-PROTOCOL.md` | reference | Evidence tiers, test strategy, stable-fix proof expectations, Bugfix Spec Lane regression proof, release evidence traceability, opt-in accessibility advisory evidence shape, sensitive gates, and false-done warnings. | Informs checks, closeout, stable-fix eligibility, release-relevant proof traces, accessibility advisory completeness after invocation, and OS Health warnings. |
...
| `tasks/reference/MEMORY-PROTOCOL.md` | reference | Reviewed filesystem memory rules, selective recall behavior, token-budget warnings, generated index/search behavior, citation expectations, demotion warnings, and manual promotion path. | Governs `memory/cards/`, `scripts/memory-check.py`, and generated memory indexes without expanding active memory or requiring retrieval-backed infrastructure. |
...
| `scripts/next-step.py` | Prints canonical generated next-step routing guidance with a plain user decision and stable-fix eligibility summary. | Compiled state. | Human-readable stdout or advisory JSON including `single_next_protocol`, `load_plan`, `context_footprint`, and `stable_fix_eligibility`; may return `review` when an `in_progress` bead has enough closeout evidence and only the review decision is missing; no state mutation. |
...
| `scripts/completion-check.py` | Prints advisory completion, handoff, release-evidence, and reference-follow-through readiness. | Active bead closeout, recorded checks, handoff fields, release evidence fields, current changed paths, generated HTML freshness cues, and maintainer-history signals. | Advisory JSON only; may warn that public reference docs, protocols, package inventory, generated HTML, `_maintainer/CHANGELOG.md`, or roadmap/journal history need review, but does not edit files, accept implementation, approve transition, update maintainer history, or make generated output authoritative. |
...
| `scripts/update-bead-build-journal.py` | Renders generated implemented-bead path and build-change journal. | Active bead, closeout, checks, tool runs, loop events, Git metadata, and optional Candidate Queue / PRD provenance. | `logs/bead-build-journal.md/jsonl`; generated evidence only, not acceptance, Candidate Queue authority, or transition approval. |
...
| `scripts/update-memory-index.py` | Renders reviewed memory index. | `memory/cards/*.md`. | `logs/memory-index.md/json` with citation, memory-space, size, and token-budget warning fields. |
| `scripts/memory-check.py` | Searches, selectively recalls, or audits reviewed memory cards. | `memory/cards/*.md`, optional query/category/freshness/status/promotion filters, optional `--recall`, optional `--self-test`. | Advisory JSON only; recall returns concise cited snippets and explicit misses; no card creation, owner-file promotion, task selection, or state mutation. |
| `scripts/docs-html.py` | Generates or checks the committed public HTML reading surface. | `docs/*.md` and selected root public docs such as `CANDIDATE-QUEUE.md`; optional `--check`. | `docs-html/*.html`; generated convenience only, not Markdown authority, Candidate Queue promotion, PRD approval, bead activation, task selection, or active memory. |
...
| `logs/bead-build-journal.md/jsonl` | generated report | Session-close implemented-bead path, bead-level build-change journal, optional Candidate Queue / PRD provenance, and evidence-backed implementation snapshot. | Not active memory, Candidate Queue authority, task authority, acceptance, or transition approval. |
