...
Document version: v0.7.107
Last updated: 2026-09-06

## 1. Start Here
<!-- ANCHOR: guide-part-1-start-here -->

This guide is easier to use as an action map than as a book. Start in the Daily Cockpit for normal work, then come here only for the deeper manual section that matches the action you need. The generated HTML page is the best reading surface because it gives you source links, section anchors, progress, and an on-page table of contents; this Markdown file remains the canonical source.
...
For the full document compass, go back to `README.md`. For day-to-day work, start with `docs/PRECODE-DAILY-COCKPIT.md`. Its first-class sections are Daily Loop, Next, Health, and Diary; complete prompts and protocols stay in their named owner files and are loaded only when needed. When you use Codex, Claude Code, Cursor, Gemini, or another host coding agent, the cockpit is where you decide what to ask, what state and proof matter, what approval remains, and when to stop. This guide is the deeper operating manual: use it only when the cockpit points you here or when you need more context before approving risk. Do not treat it as a second start page.

If PrecodeOS is not set up in your project yet, start with `docs/PRECODE-GUIDED-SETUP.md`. That guide walks through pulling the public PrecodeOS repo from GitHub or using the optional npm entry, running Bootstrap Confidence, choosing the first adoption fork, copying the public package files into a fresh project or using Existing Repo Intake for an existing app, excluding private and generated material, and validating before work starts. Before using `npx @precodeos/precodeos ...` in setup or support, check availability with `npm view @precodeos/precodeos version`; if npm is unavailable, unverified, returns 404, or fails on a live call, use the adjacent Python/local checkout command instead. If PrecodeOS is already embedded in your project and you want to refresh package-owned surfaces, use the Existing Precode Refresh prompt in Guided Setup, `npx @precodeos/precodeos upgrade-preview --target <existing-precode-root>`, `npx @precodeos/precodeos update-plan-preview --target <existing-precode-root>`, `npx @precodeos/precodeos apply-package-owned --target <existing-precode-root> --approve-action <UP-ID>`, or `tasks/reference/PROMPT-PATTERNS.md`. It previews first and stops before approved `UP-ID` copy actions. Upgrade preview may show advisory package version, stable/latest/pinned language, and updater compatibility policy metadata. Update-plan preview may group current `UP-ID` evidence with same-session freshness and validation prompts. `apply-package-owned` delegates only approved missing package-owned copy actions to the Python upgrade-apply path. PrecodeOS commands perform no registry lookup or dist-tag resolution; the external availability preflight is command availability only and does not make `latest` overwrite permission, update permission, or package-manager behavior.
...
Read only the part that matches your current action. If a section names a protocol, report, or prompt catalog, treat it as a routed reference, not a new start page.
...
<!-- ANCHOR: guide-part-2-choose-the-right-action -->

Use this part when you know the kind of help you need but not the exact Precode path. It consolidates workflow choice, prompt choice, parked intent, and future-work filing into one action-first lookup.
...
| Approved PRD may need handoff | PRD Handoff Readiness Packet | `Run python3 scripts/prd-handoff-readiness.py --prd <path> --target general and summarize whether this PRD is ready for decomposition, design, engineering, or review handoff. Include status, requirement IDs, open questions, acceptance coverage, candidate bead readiness, proof expectations, risks, owner protocols, Progressive Production-Readiness Activation, blockers, and next safe action. Treat the packet as generated evidence only. Do not approve the PRD, rewrite owner files, choose tasks, activate beads, accept implementation, certify production readiness, mutate external tools, automate exports, create MCP behavior, create registries, or code.` |
| Approved PRD has architecture, API, data, security, codebase, or verification risk | Progressive Production-Readiness Activation | `Run python3 scripts/prd-handoff-readiness.py --prd <path> --target decomposition. Show details.packet.production_readiness_activation and explain which existing owner files activate, what is missing, what proof is expected, and the next safe action. Use explicit PRD fields only. Do not treat the packet as application analysis, owner-file edit approval, decomposition approval, generated proof, or production-readiness certification.` |
...
| Security, release, docs freshness, dependency, engineering-quality, polish/product-taste, draft-PRD quality, package cross-reference/staleness, or multi-risk review needs a named review lens | Review Lane | `Use the Review Lanes Protocol. Run exactly one lane, or use Multi-Lane Review Invocation for one named active bead or draft PRD with 2-3 selected lanes. Show findings, missing proof, acceptance questions, recommendation, approval still required, and promotion path. Do not approve review, PRDs, release, security, compliance, design quality, UX quality, code quality, production readiness, transitions, parallel execution, declare stale claims authoritative, rewrite owner files, create product authority from reviewer taste, create generated proof, create generated authority, or create tasks.` |
...
## 3. Shape Ideas Before Coding
<!-- ANCHOR: guide-part-3-shape-ideas-before-coding -->

Use this part before durable product work exists or before a rough idea becomes a PRD, bead, or implementation plan. The output should be evidence, clarity, and reviewed intent, not coding permission.
...
13. If later approved PRD-like input and Experience artifacts exist, complete `tasks/templates/BUILDER-EXPERIENCE-INGESTION-PACKET.md` before Claude Code proposes the first implementation bead.
...
Use the Exploration Loop when you have already collected material and want help thinking with it before committing to a PRD. This is especially useful for small MVPs, scattered workbook notes, rough feature lists, user quotes, research snippets, screenshots, sketches, chat summaries, Candidate Goal Frames, or prior not-yet ideas.
...
Use Build-React-Learn when a builder needs to try a small product option in the real repo before deciding whether that approach is the right path.
...
Use this part once PrecodeOS is installed and you are starting, checking, steering, or bounding an active session. It keeps the agent tied to the active bead, authority file, checks, and stop conditions.
...
If `next-step.py` says `author next bead`, the current bead is in an accepted hold: evidence and review are complete, but the next bead still needs to be scoped or authored before transition approval. If the output shows next-work source reconciliation, ask the agent to compare the router-proposed next bead, Closeout `next_bead`, Handback or Noticed recommendation, and existing bead-file readiness before naming the next action. Do not keep implementing the accepted bead or repeat the acceptance review. Ask the agent to propose or author the next bead, then show the transition proposal without approving it.
...
## 5. Prove, Review, Close, Or Release
<!-- ANCHOR: guide-part-5-prove-review-close-or-release -->

Use this part when work may be done, risky, ready for review, or approaching release. Generated reports remain evidence only; human review and explicit approval still decide acceptance, transition, merge, and release.
...
Use a Review Lane when one active bead, one draft PRD, or one bounded package documentation/reference surface needs a named specialist review question without turning that specialist into a fake teammate, product approver, or approval authority.
...
Use Polish / Product-Taste Review Lane for a completed or nearly completed user-facing active bead when visible user flow, screen behavior, copy clarity, target-user comprehension, acceptance-oracle fit, manual QA path, roughness, or remaining human taste judgment needs review.
...
Use Multi-Lane Review Invocation when one named active bead or draft PRD has 2-3 concrete review risks and choosing exactly one lane would under-review the target. Ask the agent to recommend up to 3 lanes or name the lanes yourself. If the requested bundle is too broad, the agent should recommend the smaller lane set before reviewing.
...
For Polish / Product-Taste Review Lane, the lane reviews visible experience risk as advisory input only. It does not certify design quality or UX quality, approve acceptance or release, make reviewer taste product authority, treat screenshots or browser notes as proof by themselves, create follow-up tasks, require professional UX research, create a generated report, or become a required gate.
...
For Multi-Lane Review Invocation, each selected lane still follows its normal rules. The merged output groups findings by lane and deduplicates missing proof, acceptance questions, recommendation, approval still required, and promotion path. It is not a new lane, generated report, checker, scorecard, subagent pattern, command wrapper, task runner, approval gate, generated proof, generated authority, or parallel-agent authority.

Stop if: the agent treats the lane or multi-lane invocation as acceptance, PRD approval, PRD rewrite permission, release approval, security sign-off, compliance approval, design quality or UX quality certification, code-quality certification, production-readiness certification, transition approval, parallel execution approval, stale-claim authority, Work Graph authority, generated HTML authority, scorecard authority, checker authority, generated proof, generated authority, or a task creator.
...
## 6. Recover, Learn, And Look Up Reference
<!-- ANCHOR: guide-part-6-recover-learn-and-look-up-reference -->

Use this part when something feels broken, a file or term is confusing, a lesson should survive, or you need compact reference material. Recovery and learning stay evidence-first and approval-gated.
