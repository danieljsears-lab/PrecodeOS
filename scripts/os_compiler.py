...
from precode_inventory import (
    ACTIVE_MEMORY,
    ADAPTER_DOCS,
    SHIM_DOCS,
    compile_adapter_index,
    compile_authority_map,
    compile_file_inventory,
    compile_shim_index,
    file_size,
)
...
def compact_digest_text(value: Any, limit: int = 180) -> str:
...
def latest_check_digest(check_results: list[dict[str, Any]]) -> str:
    if not check_results:
        return "no recorded checks yet"
    latest = check_results[-1]
    command = compact_digest_text(latest.get("command") or "recorded check", 90)
    status = str(latest.get("status") or "unknown")
    exit_code = latest.get("exit_code")
    return f"`{command}` {status} (exit {exit_code})"


def changed_paths_digest(changed_paths: list[str] | None = None, changed_summary: str = "") -> str:
    if changed_summary:
        return compact_digest_text(changed_summary, 100)
    paths = [str(path) for path in (changed_paths or []) if str(path).strip()]
    if not paths:
        return "none recorded"
    if len(paths) <= 3:
        return ", ".join(f"`{path}`" for path in paths)
    return f"{len(paths)} changed path(s), including " + ", ".join(f"`{path}`" for path in paths[:3])


def closeout_work_digest(
    bead: BeadRecord,
    *,
    ...
    objective = compact_digest_text(first_bullet(bead.sections.get("Objective", "")) or bead.title, 120)
    changed = changed_paths_digest(changed_paths, changed_summary)
    proof = latest_check_digest(check_results or [])
    manual = compact_digest_text(bead.closeout.get("manual_verification", "not recorded"), 100)
    follow_up = compact_digest_text(bead.closeout.get("follow_up_bead_needed", "not evaluated"), 100)
    review = compact_digest_text(bead.closeout.get("review_decision", "not reviewed"), 80)
    next_action = compact_digest_text(
        next_safe_action or bead.closeout.get("next_bead", "") or "not evaluated",
        120,
    )
    return (
        f"{bead.title}: {objective}. "
        f"Changed: {changed}. "
        f"Proof: {proof}; manual verification: {manual}. "
        f"Parked/follow-up: {follow_up}. "
        f"Approval: {review}. "
        f"Next safe action: {next_action}."
    )


def commit_subject_fragment(value: str) -> str:
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    text = re.sub(r"^B\d+\s*(?:[-:—–])\s*", "", text)
    text = text.strip(" .")
    if not text:
        return "active bead work"
    return text[0].lower() + text[1:]


def advisory_commit_message_suggestion(
    bead: BeadRecord,
    *,
    changed_paths: list[str] | None = None,
    check_results: list[dict[str, Any]] | None = None,
    ...
    bead_rows = check_results or []
    objective = first_bullet(bead.sections.get("Objective", "")) or bead.title
    subject = compact_digest_text(f"Complete {commit_subject_fragment(objective)}", 72)
    changed = changed_paths_digest(changed_paths, changed_summary)
    proof = latest_check_digest(bead_rows)
    manual = compact_digest_text(bead.closeout.get("manual_verification", "not recorded"), 120)
    review = compact_digest_text(bead.closeout.get("review_decision", "not reviewed"), 100)
    follow_up = compact_digest_text(bead.closeout.get("follow_up_bead_needed", "not evaluated"), 120)
    next_action = compact_digest_text(
        next_safe_action or bead.closeout.get("next_bead", "") or "not evaluated",
        120,
    )
    body = "\n".join(
        [
            "Advisory draft only; review before use.",
            "",
            f"Summary: {compact_digest_text(closeout_work_digest(bead, changed_paths=changed_paths, check_results=bead_rows, next_safe_action=next_action, changed_summary=changed_summary), 220)}",
            f"Changed: {changed}",
            f"Checks: {proof}",
...
def inline_commit_message_suggestion(suggestion: dict[str, Any]) -> str:
    subject = compact_digest_text(suggestion.get("subject") or "not recorded", 90)
    body = compact_digest_text(suggestion.get("body") or "not recorded", 240)
    return f"Subject: {subject} | Body: {body} | Advisory only; does not stage, commit, push, accept review, or approve transition."
...
def completion_handoff_quality(
    ...
    required_context["work digest"] = closeout_work_digest(
        bead,
    ...
    work_digest = required_context["work digest"]
    commit_suggestion = advisory_commit_message_suggestion(
        bead,
        changed_paths=changed,
        check_results=bead_rows,
        next_safe_action=next_safe_action,
