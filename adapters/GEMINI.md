...
Follow the **V1 Agent-Operated Setup Contract** in `adapters/ADAPTER-INDEX.md`. Do not introduce Gemini-specific setup semantics or approval rules.
...
Use `tasks/reference/AGENT-ROUTING-PROTOCOL.md` for shared routing rules before choosing Gemini-specific controls.

Map `fast`, `default`, `deep`, and `long-horizon` to the available Gemini model, effort, context, or delegation controls only when the active tool exposes them. If Gemini does not expose a native compaction threshold or delegation control, fall back to Precode checkpoint, Context Pack, handoff, and fresh-context review discipline.
