...
- Manual verification should use the Verification Guardrail Protocol format: who checked, what was checked, environment, result, and remaining uncertainty. Closeout refresh must preserve multiline field content; ordinary prose such as `failures`, `No missing fields`, or `nothing was blocked` is not pending status, while explicit `pending`, `missing`, `blocked`, or failed status remains review-blocking.
