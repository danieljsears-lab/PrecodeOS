...
Curated pack exemplar boundary: `tasks/templates/RELEASE-READINESS-CURATED-PACK-EXEMPLAR.md` groups the Release Readiness Protocol, verification, closeout, prompt, and host-skill packaging surfaces as the official non-installable exemplar review shape. This grouping does not install a skill, create optional-pack behavior, create a registry or marketplace, add command-wrapper behavior, approve release, deploy, mutate providers, add release-channel behavior, or add package-manager behavior.
```
