...
Keep builder-facing workflow selection subordinate to the first-reader route: not installed goes to Guided Setup; installed or working goes to Daily Cockpit; rough ideas use the Daily Cockpit `Ideation: use First PRD Walkthrough for my rough idea.` path; broken or confusing setup, state, checks, or generated reports go to Troubleshooting or `I am stuck, help me.` README is the public compass, User Guide is the annex, How-To is the educational bridge, Ask Precode and Artifact Chooser are conditional helpers, and Release Readiness is release-prep rather than deployment automation. Beginner-facing docs may show advanced trigger summaries, but detailed advanced prompts belong in Prompt Patterns or the owner protocol and must not be presented as peer routes.
