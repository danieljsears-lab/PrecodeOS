...
The approved copy set may include `.gitignore` when the target lacks one so generated bytecode, caches, local tooling state, generated reports, and secrets are ignored before the first target commit. Existing `.gitignore` files must be preserved or adapted manually, not overwritten.
