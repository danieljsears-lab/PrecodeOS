...
PrecodeOS's existing owner files, protocols, scripts, generated sidecars, recorded checks, adapters, and transparent command facades form an advisory repo-native harness contract. Treat harness hardening as boundary and contract work, not as permission to add a Harness Protocol, agent runtime, sandbox, command approval layer, registry, optional pack, package manager, install/update system, or enforcement layer. The optional npm `precodeos` entry is allowed as transparent setup/upgrade/update-plan preview delegation plus approved package-owned copy delegation over Bootstrap Confidence; it may show advisory local release-reference metadata, updater compatibility policy metadata, grouped update-plan metadata, and `apply-package-owned` delegation for explicit current `UP-ID` approvals, but it must not add postinstall behavior, JavaScript copy logic, dirty-file overwrite, owner-file adaptation, registry lookup, dist-tag resolution, executable release-channel behavior, package-manager updates, or a support-only hidden install process. Updater compatibility policy is owned by `tasks/prds/PRD-041-npm-updater-evidence-and-compatibility-policy.md`, update-plan preview is owned by `tasks/prds/PRD-042-npm-update-plan-preview.md`, approved package-owned apply is owned by `tasks/prds/PRD-047-npm-approved-package-owned-apply.md`, and none approves extensions, commands, broad updater behavior, or package-manager behavior.
