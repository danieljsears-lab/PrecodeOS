...
| Empty target | Show public file-group setup candidates, including `.gitignore`, owner-file adaptation candidates, exclusions, approval gates, and validation steps. |
...
- It must include `.gitignore` as a reviewed setup surface when absent so generated bytecode, caches, local tooling state, generated reports, and secrets are ignored before the first target commit.
