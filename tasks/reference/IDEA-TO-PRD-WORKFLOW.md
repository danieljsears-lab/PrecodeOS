...
Use `tasks/reference/ARCHITECTURE-SHAPING-PROTOCOL.md` after PRD approval and before bead proposals when the approved destination touches auth, data models, APIs, integrations, dependencies, migrations, external services, multi-step workflows, or multi-system changes. Architecture Shaping is a conditional bridge, not a universal prerequisite for every bead. If the approved PRD is low-risk, continue to bead decomposition and record the skip reason in the PRD architecture-impact section or bead notes.
...
| `FEATURES.md` -> Architecture Shaping when risk-triggered, or recorded low-risk skip | Produce an evidence-only Architecture Brief before decomposition only for architecture-sensitive approved PRDs. | Do not make Architecture Shaping a universal bead prerequisite, create a full implementation plan, or promote brief facts into owner files without review. | Hidden auth, data, API, integration, migration, dependency, workflow, or multi-system risks are surfaced, or a low-risk skip reason is recorded in PRD or bead notes. | Product-changing technical risk appears, owner-file facts need promotion, or an unknown needs a planning/unblocker bead. |
...
Use the Plan Mode Candidate Craft Loop when an idea or feature angle should become future candidate work before implementation: `Idea -> Plan Mode -> Candidate Queue -> Plan Mode -> Implementation Plan -> Approved Bead -> Build`. Plan Mode is required before developing a Candidate Queue entry and again before developing an implementation plan for a selected candidate. In Codex, use `/plan`; in Claude Code, use Plan Mode; in other agents, use an equivalent read-only planning mode.

Use Plan Loop after Local Source Intake, during PRD shaping, after PRD approval, or before candidate bead activation when the user explicitly asks to think through a feature angle before committing it. Use Plan Mode first when available. The Plan Loop should ask one targeted question at a time only when the answer could change the next workflow, risk, first slice, owner-file impact, or stop condition.

The Plan Loop output is a Plan Packet, not a PRD, backlog, requirements section, bead contract, implementation plan approval, or implementation permission. Before PRD approval, it may recommend Product Discovery, PRD draft or amendment, Candidate Queue, owner-file update, or stop, but no bead proposal. After an approved PRD, it may sketch a candidate first-slice shape, but Decomposition must create any candidate bead proposal. Before candidate activation, it may challenge or refine the proposal, but it must not update `tasks/todo.md`, activate work, or code. If a selected candidate needs an implementation plan, produce that plan in Plan Mode and keep it evidence-only until PRD, owner-file, Architecture Shaping, Decomposition, and transition gates are satisfied.
