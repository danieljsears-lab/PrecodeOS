...
`scripts/version-check.py` enforces public package version metadata coverage. It reports missing or malformed version metadata as JSON and exits nonzero when drift is found, without mutating files.
...
Findings are generated evidence only. They do not choose tasks, approve PRDs, activate beads, change bead state, or edit active memory.
