...
Selective recall returns snippets only when every query term matches the reviewed card search text. Recall JSON may include compact citation, demotion decision, and filing recommendation fields so an agent can cite the card, explain whether the result is demoted, and recommend the smallest destination. If there is no exact match, weak-match examples are leads only; do not load them as memory or treat them as recall.
...
Search results and generated indexes are evidence only. Cite the card path, title, category, freshness, status, source pointers, and promotion owner before using a result. Demote stale, superseded, archived, low-confidence, or `needs_promotion` cards, then return to active memory, the active bead, and the owner file before recommending action. Filing recommendations may suggest staying reviewed memory, proposing a memory card, promoting to an owner file, or deferring, but they do not file automatically.
